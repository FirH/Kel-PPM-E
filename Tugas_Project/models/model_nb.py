...
def hitung_rata2_std_kelas(input_data, num):
    ...
    for column in list_columns:
        for a_class in list_class:
            rata2[(a_class, column)] = input_data.loc[
                input_data[class_column_name] == a_class
            ][column].mean()
            std[(a_class, column)] = input_data.loc[
                input_data[class_column_name] == a_class
            ][column].std()
    return (rata2, std)
...
def hitung_likelihood_gaussian(data, rata2, std):
    hasil = (1 / math.sqrt(2 * math.pi * (std**2))) * math.exp(
        (-1 * ((data - rata2) ** 2)) / (2 * (std**2))
    )
    return hasil
        ...
        for a_class in list_class:
            if column in cat:
                ...
                for i in range(data_latih.shape[0]):
                    likelihood[(column, np.NaN, a_class)] = hitung_likelihood_gaussian(
                        data_latih[column].iloc[i],
    ...
    for column in list_columns:
        if column in cat_columns:
            for a_class in list_class:
                posterior[a_class] = (
                    posterior[a_class]
                    * likelihood[(column, data_uji[column], a_class)]
                    * prior[a_class]
                )
            ...
            for a_class in list_class:
                posterior[a_class] = (
                    posterior[a_class]
                    * likelihood[(column, np.NaN, a_class)]
                    * prior[a_class]
                )
